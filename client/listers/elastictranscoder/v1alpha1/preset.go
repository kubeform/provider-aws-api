...
// List lists all Presets in the indexer.
func (s *presetLister) List(selector labels.Selector) (ret []*v1alpha1.Preset, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Preset))
	})
	return ret, err
}
...
// List lists all Presets in the indexer for a given namespace.
func (s presetNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Preset, err error) {
...
// Get retrieves the Preset from the indexer for a given namespace and name.
func (s presetNamespaceLister) Get(name string) (*v1alpha1.Preset, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("preset"), name)
	}
	return obj.(*v1alpha1.Preset), nil
}
