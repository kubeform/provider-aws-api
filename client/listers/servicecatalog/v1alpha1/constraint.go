...
type ConstraintLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.Constraint, err error)
...
// List lists all Constraints in the indexer.
func (s *constraintLister) List(selector labels.Selector) (ret []*v1alpha1.Constraint, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.Constraint))
	})
	return ret, err
}
...
type ConstraintNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.Constraint, error)
...
// List lists all Constraints in the indexer for a given namespace.
func (s constraintNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.Constraint, err error) {
...
// Get retrieves the Constraint from the indexer for a given namespace and name.
func (s constraintNamespaceLister) Get(name string) (*v1alpha1.Constraint, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("constraint"), name)
	}
	return obj.(*v1alpha1.Constraint), nil
}
