...
// cloudtrailLister implements the CloudtrailLister interface.
...
func NewCloudtrailLister(indexer cache.Indexer) CloudtrailLister {
	return &cloudtrailLister{indexer: indexer}
}

// List lists all Cloudtrails in the indexer.
func (s *cloudtrailLister) List(selector labels.Selector) (ret []*v1alpha1.Cloudtrail, err error) {
...
func (s cloudtrailNamespaceLister) Get(name string) (*v1alpha1.Cloudtrail, error) {
	...
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("cloudtrail"), name)
	}
	return obj.(*v1alpha1.Cloudtrail), nil
}
