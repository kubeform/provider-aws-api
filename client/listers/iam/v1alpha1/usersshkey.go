...
type UserSSHKeyLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.UserSSHKey, err error)
...
// List lists all UserSSHKeys in the indexer.
func (s *userSSHKeyLister) List(selector labels.Selector) (ret []*v1alpha1.UserSSHKey, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.UserSSHKey))
	})
	return ret, err
}
...
type UserSSHKeyNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.UserSSHKey, error)
...
// List lists all UserSSHKeys in the indexer for a given namespace.
func (s userSSHKeyNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.UserSSHKey, err error) {
...
// Get retrieves the UserSSHKey from the indexer for a given namespace and name.
func (s userSSHKeyNamespaceLister) Get(name string) (*v1alpha1.UserSSHKey, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("usersshkey"), name)
	}
	return obj.(*v1alpha1.UserSSHKey), nil
}
