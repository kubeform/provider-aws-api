/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1alpha1 "kubeform.dev/provider-aws-api/apis/chime/v1alpha1"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
)

// FakeVoiceConnectors implements VoiceConnectorInterface
type FakeVoiceConnectors struct {
	Fake *FakeChimeV1alpha1
	ns   string
}

var voiceconnectorsResource = schema.GroupVersionResource{Group: "chime.aws.kubeform.com", Version: "v1alpha1", Resource: "voiceconnectors"}

var voiceconnectorsKind = schema.GroupVersionKind{Group: "chime.aws.kubeform.com", Version: "v1alpha1", Kind: "VoiceConnector"}

// Get takes name of the voiceConnector, and returns the corresponding voiceConnector object, and an error if there is any.
func (c *FakeVoiceConnectors) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.VoiceConnector, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(voiceconnectorsResource, c.ns, name), &v1alpha1.VoiceConnector{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.VoiceConnector), err
}

// List takes label and field selectors, and returns the list of VoiceConnectors that match those selectors.
func (c *FakeVoiceConnectors) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.VoiceConnectorList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(voiceconnectorsResource, voiceconnectorsKind, c.ns, opts), &v1alpha1.VoiceConnectorList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.VoiceConnectorList{ListMeta: obj.(*v1alpha1.VoiceConnectorList).ListMeta}
	for _, item := range obj.(*v1alpha1.VoiceConnectorList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested voiceConnectors.
func (c *FakeVoiceConnectors) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(voiceconnectorsResource, c.ns, opts))

}

// Create takes the representation of a voiceConnector and creates it.  Returns the server's representation of the voiceConnector, and an error, if there is any.
func (c *FakeVoiceConnectors) Create(ctx context.Context, voiceConnector *v1alpha1.VoiceConnector, opts v1.CreateOptions) (result *v1alpha1.VoiceConnector, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(voiceconnectorsResource, c.ns, voiceConnector), &v1alpha1.VoiceConnector{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.VoiceConnector), err
}

// Update takes the representation of a voiceConnector and updates it. Returns the server's representation of the voiceConnector, and an error, if there is any.
func (c *FakeVoiceConnectors) Update(ctx context.Context, voiceConnector *v1alpha1.VoiceConnector, opts v1.UpdateOptions) (result *v1alpha1.VoiceConnector, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(voiceconnectorsResource, c.ns, voiceConnector), &v1alpha1.VoiceConnector{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.VoiceConnector), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeVoiceConnectors) UpdateStatus(ctx context.Context, voiceConnector *v1alpha1.VoiceConnector, opts v1.UpdateOptions) (*v1alpha1.VoiceConnector, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(voiceconnectorsResource, "status", c.ns, voiceConnector), &v1alpha1.VoiceConnector{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.VoiceConnector), err
}

// Delete takes name of the voiceConnector and deletes it. Returns an error if one occurs.
func (c *FakeVoiceConnectors) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(voiceconnectorsResource, c.ns, name), &v1alpha1.VoiceConnector{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeVoiceConnectors) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(voiceconnectorsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.VoiceConnectorList{})
	return err
}

// Patch applies the patch and returns the patched voiceConnector.
func (c *FakeVoiceConnectors) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.VoiceConnector, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(voiceconnectorsResource, c.ns, name, pt, data, subresources...), &v1alpha1.VoiceConnector{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.VoiceConnector), err
}
