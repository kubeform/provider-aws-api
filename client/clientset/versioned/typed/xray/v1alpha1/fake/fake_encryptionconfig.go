/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
	v1alpha1 "kubeform.dev/provider-aws-api/apis/xray/v1alpha1"
)

// FakeEncryptionConfigs implements EncryptionConfigInterface
type FakeEncryptionConfigs struct {
	Fake *FakeXrayV1alpha1
	ns   string
}

var encryptionconfigsResource = schema.GroupVersionResource{Group: "xray.aws.kubeform.com", Version: "v1alpha1", Resource: "encryptionconfigs"}

var encryptionconfigsKind = schema.GroupVersionKind{Group: "xray.aws.kubeform.com", Version: "v1alpha1", Kind: "EncryptionConfig"}

// Get takes name of the encryptionConfig, and returns the corresponding encryptionConfig object, and an error if there is any.
func (c *FakeEncryptionConfigs) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.EncryptionConfig, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(encryptionconfigsResource, c.ns, name), &v1alpha1.EncryptionConfig{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EncryptionConfig), err
}

// List takes label and field selectors, and returns the list of EncryptionConfigs that match those selectors.
func (c *FakeEncryptionConfigs) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.EncryptionConfigList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(encryptionconfigsResource, encryptionconfigsKind, c.ns, opts), &v1alpha1.EncryptionConfigList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.EncryptionConfigList{ListMeta: obj.(*v1alpha1.EncryptionConfigList).ListMeta}
	for _, item := range obj.(*v1alpha1.EncryptionConfigList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested encryptionConfigs.
func (c *FakeEncryptionConfigs) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(encryptionconfigsResource, c.ns, opts))

}

// Create takes the representation of a encryptionConfig and creates it.  Returns the server's representation of the encryptionConfig, and an error, if there is any.
func (c *FakeEncryptionConfigs) Create(ctx context.Context, encryptionConfig *v1alpha1.EncryptionConfig, opts v1.CreateOptions) (result *v1alpha1.EncryptionConfig, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(encryptionconfigsResource, c.ns, encryptionConfig), &v1alpha1.EncryptionConfig{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EncryptionConfig), err
}

// Update takes the representation of a encryptionConfig and updates it. Returns the server's representation of the encryptionConfig, and an error, if there is any.
func (c *FakeEncryptionConfigs) Update(ctx context.Context, encryptionConfig *v1alpha1.EncryptionConfig, opts v1.UpdateOptions) (result *v1alpha1.EncryptionConfig, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(encryptionconfigsResource, c.ns, encryptionConfig), &v1alpha1.EncryptionConfig{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EncryptionConfig), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeEncryptionConfigs) UpdateStatus(ctx context.Context, encryptionConfig *v1alpha1.EncryptionConfig, opts v1.UpdateOptions) (*v1alpha1.EncryptionConfig, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(encryptionconfigsResource, "status", c.ns, encryptionConfig), &v1alpha1.EncryptionConfig{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EncryptionConfig), err
}

// Delete takes name of the encryptionConfig and deletes it. Returns an error if one occurs.
func (c *FakeEncryptionConfigs) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(encryptionconfigsResource, c.ns, name), &v1alpha1.EncryptionConfig{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeEncryptionConfigs) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(encryptionconfigsResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.EncryptionConfigList{})
	return err
}

// Patch applies the patch and returns the patched encryptionConfig.
func (c *FakeEncryptionConfigs) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.EncryptionConfig, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(encryptionconfigsResource, c.ns, name, pt, data, subresources...), &v1alpha1.EncryptionConfig{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.EncryptionConfig), err
}
