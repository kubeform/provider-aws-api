/*
Copyright AppsCode Inc. and Contributors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package fake

import (
	"context"

	v1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	labels "k8s.io/apimachinery/pkg/labels"
	schema "k8s.io/apimachinery/pkg/runtime/schema"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	testing "k8s.io/client-go/testing"
	v1alpha1 "kubeform.dev/provider-aws-api/apis/cognito/v1alpha1"
)

// FakeResourceServers implements ResourceServerInterface
type FakeResourceServers struct {
	Fake *FakeCognitoV1alpha1
	ns   string
}

var resourceserversResource = schema.GroupVersionResource{Group: "cognito.aws.kubeform.com", Version: "v1alpha1", Resource: "resourceservers"}

var resourceserversKind = schema.GroupVersionKind{Group: "cognito.aws.kubeform.com", Version: "v1alpha1", Kind: "ResourceServer"}

// Get takes name of the resourceServer, and returns the corresponding resourceServer object, and an error if there is any.
func (c *FakeResourceServers) Get(ctx context.Context, name string, options v1.GetOptions) (result *v1alpha1.ResourceServer, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(resourceserversResource, c.ns, name), &v1alpha1.ResourceServer{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ResourceServer), err
}

// List takes label and field selectors, and returns the list of ResourceServers that match those selectors.
func (c *FakeResourceServers) List(ctx context.Context, opts v1.ListOptions) (result *v1alpha1.ResourceServerList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(resourceserversResource, resourceserversKind, c.ns, opts), &v1alpha1.ResourceServerList{})

	if obj == nil {
		return nil, err
	}

	label, _, _ := testing.ExtractFromListOptions(opts)
	if label == nil {
		label = labels.Everything()
	}
	list := &v1alpha1.ResourceServerList{ListMeta: obj.(*v1alpha1.ResourceServerList).ListMeta}
	for _, item := range obj.(*v1alpha1.ResourceServerList).Items {
		if label.Matches(labels.Set(item.Labels)) {
			list.Items = append(list.Items, item)
		}
	}
	return list, err
}

// Watch returns a watch.Interface that watches the requested resourceServers.
func (c *FakeResourceServers) Watch(ctx context.Context, opts v1.ListOptions) (watch.Interface, error) {
	return c.Fake.
		InvokesWatch(testing.NewWatchAction(resourceserversResource, c.ns, opts))

}

// Create takes the representation of a resourceServer and creates it.  Returns the server's representation of the resourceServer, and an error, if there is any.
func (c *FakeResourceServers) Create(ctx context.Context, resourceServer *v1alpha1.ResourceServer, opts v1.CreateOptions) (result *v1alpha1.ResourceServer, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(resourceserversResource, c.ns, resourceServer), &v1alpha1.ResourceServer{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ResourceServer), err
}

// Update takes the representation of a resourceServer and updates it. Returns the server's representation of the resourceServer, and an error, if there is any.
func (c *FakeResourceServers) Update(ctx context.Context, resourceServer *v1alpha1.ResourceServer, opts v1.UpdateOptions) (result *v1alpha1.ResourceServer, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(resourceserversResource, c.ns, resourceServer), &v1alpha1.ResourceServer{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ResourceServer), err
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().
func (c *FakeResourceServers) UpdateStatus(ctx context.Context, resourceServer *v1alpha1.ResourceServer, opts v1.UpdateOptions) (*v1alpha1.ResourceServer, error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateSubresourceAction(resourceserversResource, "status", c.ns, resourceServer), &v1alpha1.ResourceServer{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ResourceServer), err
}

// Delete takes name of the resourceServer and deletes it. Returns an error if one occurs.
func (c *FakeResourceServers) Delete(ctx context.Context, name string, opts v1.DeleteOptions) error {
	_, err := c.Fake.
		Invokes(testing.NewDeleteAction(resourceserversResource, c.ns, name), &v1alpha1.ResourceServer{})

	return err
}

// DeleteCollection deletes a collection of objects.
func (c *FakeResourceServers) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	action := testing.NewDeleteCollectionAction(resourceserversResource, c.ns, listOpts)

	_, err := c.Fake.Invokes(action, &v1alpha1.ResourceServerList{})
	return err
}

// Patch applies the patch and returns the patched resourceServer.
func (c *FakeResourceServers) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *v1alpha1.ResourceServer, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewPatchSubresourceAction(resourceserversResource, c.ns, name, pt, data, subresources...), &v1alpha1.ResourceServer{})

	if obj == nil {
		return nil, err
	}
	return obj.(*v1alpha1.ResourceServer), err
}
