...
func NewCapacityProviderInformer(client versioned.Interface, namespace string, resyncPeriod time.Duration, indexers cache.Indexers) cache.SharedIndexInformer {
	return NewFilteredCapacityProviderInformer(client, namespace, resyncPeriod, indexers, nil)
}
	...
	return cache.NewSharedIndexInformer(
		&cache.ListWatch{
			ListFunc: func(options v1.ListOptions) (runtime.Object, error) {
				...
				return client.EcsV1alpha1().CapacityProviders(namespace).List(context.TODO(), options)
			},
			WatchFunc: func(options v1.ListOptions) (watch.Interface, error) {
				if tweakListOptions != nil {
					tweakListOptions(&options)
				}
				return client.EcsV1alpha1().CapacityProviders(namespace).Watch(context.TODO(), options)
			},
		},
		&ecsv1alpha1.CapacityProvider{},
		resyncPeriod,
		indexers,
	)
}
...
func (f *capacityProviderInformer) defaultInformer(client versioned.Interface, resyncPeriod time.Duration) cache.SharedIndexInformer {
	return NewFilteredCapacityProviderInformer(client, f.namespace, resyncPeriod, cache.Indexers{cache.NamespaceIndex: cache.MetaNamespaceIndexFunc}, f.tweakListOptions)
}

func (f *capacityProviderInformer) Informer() cache.SharedIndexInformer {
	return f.factory.InformerFor(&ecsv1alpha1.CapacityProvider{}, f.defaultInformer)
}

func (f *capacityProviderInformer) Lister() v1alpha1.CapacityProviderLister {
	return v1alpha1.NewCapacityProviderLister(f.Informer().GetIndexer())
}
